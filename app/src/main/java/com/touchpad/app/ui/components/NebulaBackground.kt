...
/**
 * Full-bleed, slowly rotating multi-hued nebula backdrop.
 *
 * Procedural Canvas avoids shipping large bitmaps while still delivering the
 * calming space atmosphere described in `.requirements`.
 */
@Composable
fun NebulaBackground(modifier: Modifier = Modifier) {
    ...
    LaunchedEffect(Unit) {
        while (true) {
            phase = (phase + 0.004f) % (Math.PI.toFloat() * 2f)
            delay(32L) // ~30fps is enough for a slow nebula drift.
        }
    }
